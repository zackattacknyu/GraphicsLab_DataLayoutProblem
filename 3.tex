...
misses~\cite{cacheobliviouslayout}\YOON{Year is not shown}. This measure is
...
units after the copy (bottom)\YOON{no need to have arrow in the bottom figure.
}
}
...
\YOON{This is an important paragraph, but its exposition is unclear.}
...
$k_i>0$ \YOON{why you consider the positive ones? you don't allow local relaxation by allowing negative benefit. If so, clarify this.} if the access requirement will use the new copy. Let $I$ be the set of
access requirements attached\YOON{Unclear on what you exactly meant by attached. You mean that access requirements, either one of whose two end data units is the given data?} to the data unit. Let $J$ be the set of access
requirements not in $I$ whose span overlaps our data units. We can now describe
the benefit as follows: \YOON{Why - in front of delta?}
\[
...
\For{each AR P's head and tail data units U}{
	make old copy list L' empty\; \YOON{unclear why you use this list.}
	\eIf{U is head data unit}{ \YOON{no point to introduce S. just say copy data appropriate location.}
