...
\end{figure}
\\
For the access requirement under consideration, it would not matter where in that interval we move the extremal data unit to. However, if the new location of the data block is in the span of other access requirements, it may increase the seek time of those accesses by one unit. Figure \ref{singleARafterCopyOtherAR} shows the above situation before the copying and highlights the gray access requirement that will be affected. As can be observed in figure \ref{singleARafterCopy2OtherAR} the gray access requirement has had its span increased by 1. Affecting only one access requirement, the gray one, was the least number of access requirements to affect in that interval. Therefore in general, we find a location that in the interior of the access requirement under consideration, and that is in the span of least number of access requirements. Such a location is identified using a simple linear search through the span of the access requirement.\\
